...
void print_color_add(champion_t *champion, int *beuteu)
{
...
int add(champion_t *champion)
{
    ...
    (coding_bytes) >> 4 & 3, (coding_bytes) >> 2 & 3};
    int beuteu[] = {BTR(params[0], 0),
    BTR(params[1], 0), BTR(params[2], 0)};
    int reg1 = get_vm()->arena[(champion->pc + 2) % MEM_SIZE];
    int reg2 = get_vm()->arena[(champion->pc + 2 + beuteu[0]) % MEM_SIZE];
    int reg3 = get_vm()->arena
    [(champion->pc + 2 + beuteu[0] + beuteu[1]) % MEM_SIZE];
    ...
    print_color_add(champion, beuteu);
    champion->pc =
